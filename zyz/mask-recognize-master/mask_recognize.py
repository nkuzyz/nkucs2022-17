...
class face_rec():
    ...
    def recognize(self, image_zyz):
        height, width, _ = np.shape(image_zyz)
        draw_rgb = cv2.cvtColor(image_zyz, cv2.COLOR_BGR2RGB)
        ...
        for rectangle in rectangles_temp:
            ...
            crop_img = preprocess_input(
                np.reshape(np.array(crop_img, np.float64), [1, self.Crop_HEIGHT, self.Crop_WIDTH, 3]))
        ...
        for (left, top, right, bottom), c in zip(rectangles, classes_all):
            cv2.rectangle(image_zyz, (left, top), (right, bottom), (0, 0, 255), 2)
            # image_zyz = util.christmas(image_zyz, left, top, right - left, bottom - top)
            font = cv2.FONT_HERSHEY_SIMPLEX
            cv2.putText(image_zyz, c, (left, bottom - 15), font, 0.75, (255, 255, 255), 2)
        return image_zyz
